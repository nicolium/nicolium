...
import { sortQueryData, updatePageItem } from '@/utils/queries';
...
describe('updatePageItem()', () => {
  ...
  describe('without cached data', () => {
    it('safely returns undefined', () => {
      ...
      const nextQueryData =
        queryClient.getQueryData<InfiniteData<PaginatedResponse<Item>>>(queryKey);
  ...
  describe('with cached data', () => {
    ...
    it('updates the correct item in the cached data', () => {
      const initialQueryData =
        queryClient.getQueryData<InfiniteData<PaginatedResponse<Item>>>(queryKey);
      expect(initialQueryData?.pages[0].items.map((r) => r.id)).toEqual([1, 2, 3]);
      expect(initialQueryData?.pages[0].items.find((r) => r.id === 2)?.text).toEqual('item 2');
      updatePageItem<Item>(queryKey, { id: 2, text: 'new text' }, (o, n) => o.id === n.id);
      const nextQueryData =
        queryClient.getQueryData<InfiniteData<PaginatedResponse<Item>>>(queryKey);
      expect(nextQueryData?.pages[0].items.map((r) => r.id)).toEqual([1, 2, 3]);
      expect(nextQueryData?.pages[0].items.find((r) => r.id === 2)?.text).toEqual('new text');
    });
  });
});
...
describe('sortQueryData()', () => {
  ...
  describe('with cached data', () => {
    ...
    it('sorts the cached data', () => {
      const initialQueryData =
        queryClient.getQueryData<InfiniteData<PaginatedResponse<Item>>>(queryKey);
      expect(initialQueryData?.pages[0].items[0].id === 0); // first id is 0
      sortQueryData<Item>(queryKey, (a, b) => b.id - a.id); // sort descending
      const nextQueryData =
        queryClient.getQueryData<InfiniteData<PaginatedResponse<Item>>>(queryKey);
      expect(nextQueryData?.pages[0].items[0].id === 0); // first id is now 23
    });
